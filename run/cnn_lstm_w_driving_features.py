...
def collate_fn(batch):
    ...
    trajectories = torch.cat([x[2] for x in batch], dim=0)
...
class VehicleTrajectoryModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        images, features, trajectory_camera, seq_lengths = batch
        pred_trajectory = self(images, features, seq_lengths)
        loss = self.loss_fn(pred_trajectory, trajectory_camera)
        self.log("train_loss", loss, prog_bar=True, logger=True, batch_size=images.size(0))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = self.loss_fn(pred_trajectory, trajectory_camera)
        self.log("val_loss", loss, prog_bar=True, logger=True, batch_size=images.size(0))
        return loss
...
if __name__ == "__main__":
    ...
    for fold in range(config.n_folds):
        ...
        if config.stage == "first":
            ...
            num_training_steps = (len(train_df[train_df["fold"] != fold]["scene_id"].unique()) // config.batch_size) * (config.num_epochs - 1)
            num_warmup_steps = len(train_df[train_df["fold"] != fold]["scene_id"].unique()) // config.batch_size
            trn_dl = DataLoader(trn_ds, batch_size=config.batch_size, 
