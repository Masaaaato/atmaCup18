...
if __name__ == "__main__":
    ...
    for fold in range(config.n_folds):
        ...
        test_dl = DataLoader(test_ds, batch_size=config.batch_size, shuffle=False, num_workers=config.num_workers, pin_memory=True, drop_last=False)
        ...
        # Inference
        val_preds = []
        for images, features in tqdm(val_dl, desc="Validation Inference...."):
        ...
        test_preds_fold = []
        for images, features in tqdm(test_dl, desc="Test Inference...."):
            images, features = images.to(device), features.to(device)
            with torch.no_grad():
                pred = model(images, features)
